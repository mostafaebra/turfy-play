...
const RegistrationStepper = ({ currentStep }) => {
  ...
  // حساب النسبة المئوية للشريط الأخضر
  const progressWidth =
          ...
          {steps.map((step) => {
            ...
            return (
              <div key={step.id} className="flex flex-col items-center">
                {/* الدائرة */}
                {/* bg-gray-50 ضروري عشان يغطي الخط اللي معدي من تحت الدايرة */}
                <div
                  className={`w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold border-2 transition-all duration-300
                    ${
                      isCompleted
                        ? "bg-green-500 border-green-500 text-white shadow-md scale-110" // خلصت
                        : isActive
                          ? "bg-white border-green-500 text-green-600 ring-4 ring-green-100 scale-110" // احنا هنا
                          : "bg-gray-50 border-gray-300 text-gray-400" // لسه
                    }
                  `}
                >
