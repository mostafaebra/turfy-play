...
// Icons
import { 
  MapPin, Share2, Heart, Star, ChevronLeft, Calendar, ArrowLeft,
  Zap, Car, Armchair, Waves, ShieldCheck, Clock, AlertCircle, Info, Shirt, CircleDashed,
...
const FieldDetails = () => {
  ...
  // Image Slider Logic
  const displayImages = fieldData?.imagesURLs?.length > 0 
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-12">
      ...
      <div className="relative h-[400px] w-full group">
        ...
        <button
          onClick={prevImage}
          className="absolute left-4 top-1/2 -translate-y-1/2 p-3 bg-black/30 text-white rounded-full backdrop-blur-md hover:bg-white/30 transition hidden group-hover:block"
        >
          <ChevronLeft size={24} />
        </button>
        ...
        <button
          onClick={nextImage}
          className="absolute right-4 top-1/2 -translate-y-1/2 p-3 bg-black/30 text-white rounded-full backdrop-blur-md hover:bg-white/30 transition hidden group-hover:block"
        >
        ...
        <div className="absolute top-6 right-6 flex gap-3">
          <button className="p-3 bg-black/20 backdrop-blur-md rounded-full text-white hover:bg-white hover:text-green-600 transition">
            <Share2 size={20} />
          </button>
          <button className="p-3 bg-black/20 backdrop-blur-md rounded-full text-white hover:bg-white hover:text-red-500 transition">
            <Heart size={20} />
          </button>
        </div>

        {/* Field Basic Info Overlay */}
        <div ref={descriptionRef} className="absolute bottom-0 left-0 w-full p-4 md:p-8 z-10">
      ...
      <div className="container mx-auto px-4 mt-8">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="sticky top-0 md:top-[0px] z-30 bg-gray-50 pt-2 pb-1 mb-8 transition-all">
                <div className="flex items-center gap-6 md:gap-8 border-b border-gray-200 text-gray-500 font-medium overflow-x-auto scrollbar-hide">
                    {['description', 'amenities', 'reviews', 'location', 'policy'].map((tab) => (
                        <button 
                            key={tab}
                            onClick={() => {
                                const refs = { description: descriptionRef, amenities: amenitiesRef, reviews: reviewsRef, location: locationRef, policy: policyRef };
                                scrollToSection(refs[tab]);
                            }}
                            className={`pb-3 capitalize whitespace-nowrap transition-colors ${
                                activeTab === tab
            ...
            <section className="animate-fade-in-up">
              ...
              <p className="text-gray-600 leading-relaxed text-sm md:text-base">
                ...
                the premier destination for sports enthusiasts in <span className="text-gray-800">{fieldData?.address ? fieldData.address.split(',')[0] : "the area"}</span>.
                
                This facility is proud to feature a top-tier <span className="font-bold text-green-600">
                    {fieldData?.surfaceType || "Standard"}
                </span> surface, ensuring excellent playability and safety for all athletes.
                
                <br className="mb-2 block" />
                
                Whether you are organizing a competitive league or a friendly match, our field is perfectly sized for <span className="font-bold text-blue-600">
                    {fieldData?.fieldSize} games
                </span>. 
                With well-maintained grounds and a great atmosphere, we provide the perfect environment for you to enjoy the game.
              </p>
            </section>
              ...
              {fieldData?.rating && fieldData.rating.length > 0 ? (
                <div className="space-y-6">
                  ...
                  {(() => {
                    ...
                    return displayedReviews.map((review, index) => (
                        <div key={index} className="flex gap-4 items-start border-b border-gray-50 pb-6 last:border-0">
                            <img
                                src={review.imageURL || `https://ui-avatars.com/api/?name=${review.userName || "User"}&background=random`}
                                alt={review.userName}
                            ...
                            <div className="flex-1">
                                <div className="flex justify-between items-start mb-1">
                                    <div>
                                        ...
                                        <p className="text-xs text-gray-400">{review.date || "Recent"}</p>
                                    </div>
                  ...
                  {fieldData.rating.length > 5 && (
                      ...
                      >
                        {showAllReviews ? (
                            <>Show Less <ChevronLeft className="rotate-90" size={16} /></>
                        ) : (
                            <>Show all {fieldData.rating.length} reviews <ChevronRight className="rotate-90" size={16} /></>
                        )}
                      </button>
                  )}
                </div>
            ...
            <section ref={locationRef} className="border-t border-gray-100 pt-8 scroll-mt-24">
              ...
              <div className="rounded-xl overflow-hidden border border-gray-200 shadow-sm relative group h-64 md:h-80 bg-gray-100">
                {(() => {
                            ...
                            <>
                                ...
                                <div className="absolute top-4 right-4 z-10">
                                    <button
                                        ...
                                        className="bg-white text-gray-900 px-4 py-2 rounded-lg font-bold shadow-lg hover:bg-green-600 hover:text-white transition flex items-center gap-2 text-sm"
                                    >
                                        <MapPin size={16} className="text-red-500 hover:text-white" />
                                        Open in Maps
                                    </button>
                                </div>
                            </>
                ...
                {(() => {
                    ...
                    return (
                        <div className={`${policy.bg} border ${policy.border} rounded-xl p-4 flex gap-4 items-start transition-colors`}>
                            <div className={`p-2 bg-white rounded-full ${policy.iconColor} shadow-sm shrink-0`}>
                                <Clock size={20} />
                            </div>
                            <div>
            ...
            <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 sticky top-24">
              ...
              <div className="mb-6 flex justify-center">
                ...
                <DatePicker
                  ...
                  maxDate={getMaxDate()} // Prevent future dates beyond limit
