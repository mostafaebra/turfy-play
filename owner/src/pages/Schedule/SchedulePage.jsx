...
const SchedulePage = () => {
  ...
  const generateShiftSlots = (baseDateStr, data, isTomorrow = false) => {
    ...
    for (let i = startHour; i < endHour; i++) {
      ...
      if (isPastHour) {
          ...
          // Logic: Identify Online vs Offline bookings
          // Status 0, 2, "Pending", or "Online" are treated as Online Requests (Verify Modal)
  ...
  // Handle slot click actions based on status
  ...
  return (
    <div className="flex min-h-screen bg-gray-50 relative">
      ...
      <div className={`fixed inset-y-0 left-0 z-50 w-64 bg-white shadow-lg transform transition-transform duration-300 md:relative md:translate-x-0 ${isSidebarOpen ? "translate-x-0" : "-translate-x-full"}`}>
        <Sidebar />
      </div>
      ...
      <div className="flex-1 h-screen overflow-y-auto p-4 md:p-6">
        <div className="flex flex-col gap-4 mb-6">
          <div className="flex items-center gap-3">
            <button onClick={() => setIsSidebarOpen(true)} className="p-2 bg-white border rounded-lg md:hidden"><Menu size={20} /></button>
            <h1 className="text-xl md:text-2xl font-bold text-gray-800">Schedule (48H View)</h1>
          </div>
        ...
        {/* Modals linked to refresh trigger */}
